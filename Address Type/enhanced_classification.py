...
print(f"Classified {len(df) - unclassified_mask.sum()} locations by rules")
print(f"Clustering {unclassified_mask.sum()} unclassified locations...")

if unclassified_mask.sum() > 0:
    # Feature extraction
    vectorizer = TfidfVectorizer(max_features=500, stop_words='english')
    X = vectorizer.fit_transform(df.loc[unclassified_mask, 'text_features'])
    
    # Determine optimal number of clusters
    n_clusters = min(20, max(5, unclassified_mask.sum() // 100))
    
    # Cluster the unclassified locations
    kmeans = KMeans(n_clusters=n_clusters, random_state=42, n_init=10)
    cluster_labels_array = kmeans.fit_predict(X)
    ...
    df.loc[unclassified_mask, 'Cluster'] = cluster_labels_array
    df.loc[unclassified_mask, 'Category'] = df.loc[unclassified_mask, 'Cluster'].map(cluster_labels)
...
ev_categories = ['EV Supercharger', 'Tesla Destination Charger', 
                'Tesla Charging Station', 'EV Charging Station']
ev_df = df[df['Category'].isin(ev_categories)].copy()
